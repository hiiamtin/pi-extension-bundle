...
function isPrivateIPv4(ip: string): boolean {
  ...
  if (a === 172 && b >= 16 && b <= 31) return true; // private (docker bridges)
...
function isLocalAddress(ip: string): boolean {
  ...
  if (lower.startsWith("fe80:") || lower.startsWith("fc") || lower.startsWith("fd")) return true; // v6 link-local / ULA
  if (lower.startsWith("::ffff:")) return isPrivateIPv4(lower.slice(7)); // v4-mapped
  return isPrivateIPv4(ip);
}

// A host counts as local when it is a loopback/private literal or resolves
// ONLY to loopback/private addresses (e.g. an /etc/hosts pin of a public-looking
// domain to 127.0.0.1 to reach a LAN gateway directly).
async function hostIsLocal(hostname: string): Promise<boolean> {
  ...
  try {
    ...
    return false; // unresolvable → treat as remote
  }
}
...
export async function resolveBaseUrl(): Promise<string> {
  ...
  // Local targets stay on http: LAN gateways often present self-signed certs
  // (or no TLS at all) and the upgrade would break discovery with
  // DEPTH_ZERO_SELF_SIGNED_CERT for no benefit.
  if (clean.startsWith("http://") && !(await hostIsLocal(new URL(clean).hostname))) {
