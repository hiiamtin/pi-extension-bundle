...
`PI_TOK_RATE=off` keep it fully idle. The rate is theme-colored by tier like pi-web's tps badge — thresholds configurable via `PI_TOK_RATE_TIERS="fast,ok,slow"` (default `50,30,15` → accent/success/warning, error below). Test: `node scripts/tok-rate-e2e.mjs`.
...
agents/
  ...
  oracle.md        independent second opinion on hard calls (smartmode, high)
lib/
...
scripts/
  ...
  subagent-e2e.mjs subprocess/rpc/steer/wrap-up/continue/timeout/concurrency/
